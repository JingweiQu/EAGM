...
class FeaturedGraph :
  def __init__(self, node_features, node_features2, edge_features, attr_dicts):
    self._node_features = np.array(node_features)
    self._node_features2 = node_features2
  ...
  def graph_to_data_dict(self,
    ...
    edges = None
    number_of_edges = len(self._edge_features["senders"])
    if number_of_edges == 0:
      ...
      senders = self._edge_features["senders"]
      receivers = self._edge_features["receivers"]
      edges = self._edge_features["features"]
      edges2 = self._edge_features["features2"]
...
def data_dict_to_featured_graph(data_dict):
...
#  A[data_dict[SENDERS], data_dict[RECEIVERS]] = data_dict[EDGES]
  edge_features = {"senders": data_dict[SENDERS],
                   "receiver": data_dict[RECEIVERS],
                   "features": data_dict[EDGES]}
  ...
  graph = FeaturedGraph(node_features, edge_features, attr_dicts)

  return graph


def featured_graphs_to_graphs_tuple(featured_graphs,
                                    node_shape_hint=None,
                                    edge_shape_hint=None,
                                    data_type_hint=np.float32):
...
def graphs_tuple_to_featured_graphs(graphs_tuple):
  ...
  """
  return [
      data_dict_to_featured_graph(x) for x in graphs_tuple_to_data_dicts(graphs_tuple)
  ]
