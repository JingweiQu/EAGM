...
class NodesToGroupsAggregator(snt.AbstractModule):
  """Aggregates all nodes into groups."""

  def __init__(self, reducer, name="nodes_to_groups_aggregator"):
    """Initializes the NodesToGroupsAggregator module.
    ...
    * unsorted_segment_max_or_zero
    ...
    Args:
      ...
      name: The module name.
    """
    super(NodesToGroupsAggregator, self).__init__(name=name)
    self._reducer = reducer

  def _build(self, graph, idx):
    _validate_graph(graph, (NODES,),
                    additional_message="when aggregating groups from nodes.")

    if idx == 1:
      group_indices = utils_tf.get_group_indices_1(graph)
      num_groups = tf.reduce_sum(utils_tf.get_n_group_1(graph))
    if idx == 2:
      group_indices = utils_tf.get_group_indices_2(graph)
      num_groups = tf.reduce_sum(utils_tf.get_n_group_2(graph))

    return self._reducer(graph.nodes, group_indices, num_groups)
...
class EdgeBlock(snt.AbstractModule):
  ...
  def __init__(self,
    ...
    sender_size = 32 #sender_feature.shape.as_list()[1]
    self._metric1 = tf.Variable(tf.random.normal(shape=[sender_size, sender_size], dtype = tf.float64, mean=0, stddev=1),
        dtype = tf.float64,
        name='edge_metric1')
    receiver_size = 32 #receiver_feature.shape.as_list()[1]
    self._metric2 = tf.Variable(tf.random.normal(shape=[receiver_size, receiver_size], dtype=tf.float64, mean=0, stddev=1),
        dtype=tf.float64,
        name='edge_metric2')
  ...
  def _build(self, graph):
    ...
    sender_feature = broadcast_sender_nodes_to_edges(graph)
    receiver_feature = broadcast_receiver_nodes_to_edges(graph)
    correlation = tf.multiply(tf.matmul(sender_feature, self._metric1), tf.matmul(receiver_feature, self._metric2))
    edges_to_collect.append(correlation)
  ...
  #  updated_edges = self._edge_model(collected_edges)
    updated_edges = tf.nn.relu(self._edge_model(collected_edges))
    return graph.replace(edges=updated_edges)


class EdgeBlock2(snt.AbstractModule):
  """Edge block.
  ...
  def __init__(self,
    ...
    Raises:
      ValueError: When fields that are required are missing.
    """
    super(EdgeBlock2, self).__init__(name=name)
    ...
    sender_size = 16 #sender_feature.shape.as_list()[1]
    self._metric1 = tf.Variable(tf.random.normal(shape=[sender_size, sender_size], dtype = tf.float64, mean=0, stddev=1),
        dtype = tf.float64,
        name='edge_metric1')
    receiver_size = 16 #receiver_feature.shape.as_list()[1]
    self._metric2 = tf.Variable(tf.random.normal(shape=[receiver_size, receiver_size], dtype=tf.float64, mean=0, stddev=1),
        dtype=tf.float64,
        name='edge_metric2')
...
class NodeBlock(snt.AbstractModule):
  ...
  #  updated_nodes = tf.nn.leaky_relu(self._node_model(collected_nodes))
    updated_nodes = tf.nn.relu(self._node_model(collected_nodes))
    return graph.replace(nodes=updated_nodes)


class GroupBlock(snt.AbstractModule):
  """Group block.
  ...
  def __init__(self,
               ...
               name="group_block"):
    """Initializes the GroupBlock module.
    ...
    super(GroupBlock, self).__init__(name=name)

    if not (use_nodes or use_groups):
      raise ValueError("At least one of use_nodes or use_globals must be True.")

    self._use_nodes = use_nodes
    self._use_groups = use_groups

    with self._enter_variable_scope():
      self._group_model = group_model_fn()
      if self._use_nodes:
        if nodes_reducer is None:
          raise ValueError(
              "If `use_nodes==True`, `nodes_reducer` should not be None.")
        self._nodes_aggregator = NodesToGroupsAggregator(
            nodes_reducer)

  def _build(self, graph):
    """Connects the group block.
    ...
    # if self._use_nodes:
    #   _validate_graph(graph, (NODES,), "when use_nodes == True")
    #   groups_to_collect_1.append(self._nodes_aggregator(graph, 1))
    #   groups_to_collect_2.append(self._nodes_aggregator(graph, 2))
    #
    # if self._use_groups:
    #   _validate_graph(graph, (GROUPS_1,), "when use_globals == True")
    #   _validate_graph(graph, (GROUPS_2,), "when use_globals == True")
    #   groups_to_collect_1.append(graph.groups_1)
    #   groups_to_collect_2.append(graph.groups_2)
    #
    # collected_groups_1 = tf.concat(groups_to_collect_1, axis=-1)
    # updated_groups_1 = self._group_model(collected_groups_1)
    #
    # collected_groups_2 = tf.concat(groups_to_collect_2, axis=-1)
    # updated_groups_2 = self._group_model(collected_groups_2)

    updated_groups_1 = self._nodes_aggregator(graph, 1)
    updated_groups_2 = self._nodes_aggregator(graph, 2)

    return graph.replace(groups_1=updated_groups_1, groups_2=updated_groups_2)
...
class GlobalBlock(snt.AbstractModule):
  ...
  #  updated_globals = tf.nn.leaky_relu(self._global_model(collected_globals))
    updated_globals = tf.nn.relu(self._global_model(collected_globals))
