...
def _make_default_group_block_opt(group_block_opt, default_reducer):
  """Default options to be used in the GroupBlock of a generic GraphNetwork."""
  group_block_opt = dict(group_block_opt.items()) if group_block_opt else {}
  for k, v in _DEFAULT_GROUP_BLOCK_OPT.items():
    group_block_opt[k] = group_block_opt.get(k, v)
  for key in ["nodes_reducer"]:
    group_block_opt[key] = group_block_opt.get(key, default_reducer)
  return group_block_opt
...
class GraphNetwork(snt.AbstractModule):
  ...
  def __init__(self,
    ...
    group_block_opt = _make_default_group_block_opt(group_block_opt, reducer)
    ...
    with self._enter_variable_scope():
      ...
      self._group_block = blocks.GroupBlock(
          group_model_fn=group_model_fn, **group_block_opt)
      self._global_block = blocks.GlobalBlock(
  ...
  def _build(self, graph):
   ...
   # return self._global_block(self._group_block(self._node_block(self._edge_block(graph))))


class GraphNetwork2(snt.AbstractModule):
  """Implementation of a Graph Network.
  ...
  def __init__(self,
    ...
    Args:
      ...
      name: The module name.
    """
    super(GraphNetwork2, self).__init__(name=name)
    edge_block_opt = _make_default_edge_block_opt(edge_block_opt)
    node_block_opt = _make_default_node_block_opt(node_block_opt, reducer)
    group_block_opt = _make_default_group_block_opt(group_block_opt, reducer)
    global_block_opt = _make_default_global_block_opt(global_block_opt, reducer)
    ...
    with self._enter_variable_scope():
      self._edge_block = blocks.EdgeBlock2(
          edge_model_fn=edge_model_fn, **edge_block_opt)
      self._node_block = blocks.NodeBlock(
          node_model_fn=node_model_fn, **node_block_opt)
      self._group_block = blocks.GroupBlock(
...
class GraphIndependent2(snt.AbstractModule):
  """A graph block that applies models to the graph elements independently.
  ...
  def __init__(self,
    ...
    Args:
      ...
      name: The module name.
    """
    super(GraphIndependent2, self).__init__(name=name)
  ...
  def _build(self, graph):
    ...
    # globals = self._global_model(graph.globals)
    return graph.replace(
        edges=self._edge_model(graph.edges) + graph.edges2,
        nodes=self._node_model(graph.nodes) + graph.nodes2
    )


class GraphIndependent3(snt.AbstractModule):
  """A graph block that applies models to the graph elements independently.
  ...
  def __init__(self,
    ...
    Args:
      ...
      name: The module name.
    """
    super(GraphIndependent3, self).__init__(name=name)
    ...
    with self._enter_variable_scope():
      ...
      # the Edge/Node/GlobalBlock classes do.
        ...
        if edge_model_fn2 is None:
            self._edge_model2 = lambda x: x
        else:
            self._edge_model2 = snt.Module(
                lambda x: edge_model_fn2()(x), name="edge_model2")  # pylint: disable=unnecessary-lambda

        if node_model_fn2 is None:
            self._node_model2 = lambda x: x
        else:
            self._node_model2 = snt.Module(
                lambda x: node_model_fn2()(x), name="node_model2")  # pylint: disable=unnecessary-lambda
  ...
  def _build(self, graph):
    ...
    # globals = self._global_model(graph.globals)
    return graph.replace(
        edges=self._edge_model(graph.edges) + self._edge_model2(graph.edges2),
        nodes=self._node_model(graph.nodes) + self._node_model2(graph.nodes2),
        globals=self._global_model(graph.globals)
    )


class GraphIndependentReLU(snt.AbstractModule):
  """A graph block that applies models to the graph elements independently.
  ...
  def __init__(self,
    ...
    Args:
      ...
      name: The module name.
    """
    super(GraphIndependentReLU, self).__init__(name=name)
  ...
  def _build(self, graph):
    ...
    """
    nodes = tf.nn.relu(self._node_model(graph.nodes))
    edges = tf.nn.relu(self._edge_model(graph.edges))
    globals = tf.nn.relu(self._global_model(graph.globals))
    return graph.replace(
        nodes=nodes,
        edges=edges,
        globals=globals
...
class GraphIndependentReLU3(snt.AbstractModule):
  """A graph block that applies models to the graph elements independently.
  ...
  def __init__(self,
    ...
    Args:
      ...
      name: The module name.
    """
    super(GraphIndependentReLU3, self).__init__(name=name)
  ...
  def _build(self, graph):
    ...
    """
    # nodes = tf.nn.relu(self._node_model(graph.nodes))
    # edges = tf.math.sigmoid(self._edge_model(graph.edges))
    # globals = tf.nn.relu(self._global_model(graph.globals))
    nodes = tf.math.sigmoid(self._node_model(graph.nodes))
    edges = tf.math.sigmoid(self._edge_model(graph.edges))
    globals = tf.nn.relu(self._global_model(graph.globals))
    return graph.replace(
        nodes=nodes,
...
class GraphGroupAggregator(snt.AbstractModule):
  """A graph block that applies models to the graph elements independently.
  ...
  def __init__(self,
               group_model_fn=None,
               name="graph_Group_Aggregator"):
    """Initializes the GraphGroupAggregator module.
    ...
    Args:
      ...
      name: The module name.
    """
    super(GraphGroupAggregator, self).__init__(name=name)

    with self._enter_variable_scope():
      # The use of snt.Module below is to ensure the ops and variables that
      # result from the edge/node/global_model_fns are scoped analogous to how
      # the Edge/Node/GlobalBlock classes do.
      if group_model_fn is None:
        self._group_model = lambda x: x
      else:
        self._group_model = snt.Module(
            lambda x: group_model_fn()(x), name="group_model")  # pylint: disable=unnecessary-lambda
  ...
  def _build(self, graph):
    ...
    """
    group_indices_1 = utils_tf.get_group_indices_1(graph)
    num_groups_1 = tf.reduce_sum(utils_tf.get_n_group_1(graph))
    group_indices_2 = utils_tf.get_group_indices_2(graph)
    num_groups_2 = tf.reduce_sum(utils_tf.get_n_group_2(graph))

    groups_1 = tf.math.unsorted_segment_sum(graph.nodes, group_indices_1, num_groups_1)
    groups_2 = tf.math.unsorted_segment_sum(graph.nodes, group_indices_2, num_groups_2)
