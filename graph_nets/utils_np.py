...
def networkx_to_data_dict(graph_nx,
  ...
  if number_of_nodes == 0:
    ...
    try:
      ...
      nodes_group_1 = [
          x[1][GRAPH_NX_GROUP_INDICE_1]
          for x in graph_nx.nodes(data=True)
          if x[1][GRAPH_NX_GROUP_INDICE_1] is not None
      ]
      if nodes_group_1:
        if len(nodes_group_1) != number_of_nodes:
          raise ValueError(
              "Either all the nodes should have groups, or none of them")
        group_indices_1 = np.array(nodes_group_1)
        number_of_groups_1 = np.max(group_indices_1) + 1

      number_of_groups_2 = 0
      nodes_group_2 = [
          x[1][GRAPH_NX_GROUP_INDICE_2]
          for x in graph_nx.nodes(data=True)
          if x[1][GRAPH_NX_GROUP_INDICE_2] is not None
      ]
      if nodes_group_2:
        if len(nodes_group_2) != number_of_nodes:
          raise ValueError(
              "Either all the nodes should have groups, or none of them")
  ...
  groups_1 = None
  if GRAPH_NX_GROUP_FEATURE_1 in graph_nx.graph:
    groups_1 = graph_nx.graph[GRAPH_NX_GROUP_FEATURE_1]

  groups_2 = None
  if GRAPH_NX_GROUP_FEATURE_2 in graph_nx.graph:
    groups_2 = graph_nx.graph[GRAPH_NX_GROUP_FEATURE_2]
  # ==========  added finish ===============================
