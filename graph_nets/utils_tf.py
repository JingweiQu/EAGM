...
from graph_nets import utils_weighted_graph
import six
...
def placeholders_from_weighted_graphs(graphs,
  ...
  """
  with tf.name_scope(name):
    graph = utils_weighted_graph.weighted_graphs_to_graphs_tuple(graphs, node_shape_hint,
                                               edge_shape_hint,
...
#  utils_np._check_valid_sets_of_keys([gr._asdict() for gr in input_graphs])  # pylint: disable=protected-access
  utils_weighted_graph._check_valid_sets_of_keys([gr._asdict() for gr in input_graphs])  # pylint: disable=protected-access
...
#  utils_np._check_valid_sets_of_keys(data_dicts)  # pylint: disable=protected-access
  utils_weighted_graph._check_valid_sets_of_keys(data_dicts)  # pylint: disable=protected-access
  with tf.name_scope(name):
...
def get_group_indices_1(input_graphs, name="get_groups_indices_1"):
  """Returns group_indices in `input_graphs`.
  ...
  """
  with tf.name_scope(name):
    return input_graphs.group_indices_1

def get_group_indices_2(input_graphs, name="get_groups_indices_2"):
  """Returns group_indices in `input_graphs`.
  ...
  """
  with tf.name_scope(name):
    return input_graphs.group_indices_2



def get_n_group_1(input_graphs, name="get_n_group_1"):
  """Returns the vector of N_GROUP in `input_graphs`.
  ...
  """
  with tf.name_scope(name):
    return input_graphs.n_group_1

def get_n_group_2(input_graphs, name="get_n_group_2"):
  """Returns the vector of N_GROUP in `input_graphs`.
